...
- [ ] **Step 1a: Remove `Storage` from `native`'s `lib_ignore` list**

`IStorage.h` itself has zero Arduino dependency (`FakeStorage.h`, a native
test double, must be able to `#include "IStorage.h"` via PlatformIO's
normal per-library include-path resolution — not a relative `../../../`
path hack). `platformio.ini`'s `native` environment currently ignores the
whole `lib/Storage` folder wholesale, which would make `IStorage.h`
unreachable by a clean quoted include. Fix this exactly the way
`lib/MotionHardware` already solves the identical problem for
`IServoOutput.h`: stop ignoring the folder, and instead guard only the
Arduino-bound class (`PreferencesStore`) with `#ifdef ARDUINO` (Steps 2-3
below do this) — so native's LDF processes `lib/Storage` (finding
`IStorage.h` fine) without ever trying to compile `<Preferences.h>`-dependent
code for the host.

In `platformio.ini`'s `[env:native]` section, change:
...
```ini
lib_ignore = Logger, Networking, OTA
```

And update the comment above it (currently ends "...and its native tests
are how this milestone verifies request/response encoding.") by adding one
more sentence: "Storage is deliberately NOT ignored either, for the same
reason as MotionHardware: IStorage.h has zero Arduino dependency and
FakeStorage.h (a native test double) needs to include it normally;
PreferencesStore.h/.cpp (the Arduino-bound implementation) are
`#ifdef ARDUINO`-guarded instead, matching Pca9685ServoOutput.h's pattern."

- [ ] **Step 2: Extend `PreferencesStore`'s header, guarded for native**
...
- [ ] **Step 3: Implement the three new pairs, guarded for native**
...
Create `test/test_native/support/FakeStorage.h`, using a plain quoted
include for `IStorage.h` — this resolves correctly via PlatformIO's normal
per-library include path now that Step 1a stopped ignoring `lib/Storage`
for `native`; do not use a relative `../../../` path:
