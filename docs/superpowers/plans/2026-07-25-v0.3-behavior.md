...
- [ ] **Step 8b: Correct `lib/Animation/README.md`'s expression-easing claim**

Task 5 left this file saying "expression uses Linear" — inaccurate both before
this task (expression was a no-op) and after it: `animateExpression()`
routes through the same `startEyelidTransition()`/eyelid-interpolation path
every other eyelid transition uses, which `update()` hardcodes to
`EasingType::EaseInOut` — there's no separate Linear-eased path for
expression, despite what the original v0.2 design spec assumed. Replace the
"Caller-selectable easing per call" sentence:
...
```markdown
intent — gaze uses Cubic; expression blending shares the same EaseInOut
eyelid-transition path as blink/wink/sleep/wake, since it's just another
eyelid target. `EasingType::Linear` is defined but has no caller.
```
